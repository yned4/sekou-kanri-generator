...
def _render_alias_editor():
    ...
    # ── 出来形・品質管理タブ ──────────────────────────────
    with main_tabs[0]:
        st.caption("出来形一覧・品管一覧のマッチングに使用される設定です。")
        ...
        with sub_tabs[2]:
            st.markdown("##### 出来形管理の自動追加")
            st.caption("数量総括表に特定のキーワードが含まれる場合、"
                       "関連する出来形管理工種を自動的に追加します。"
                       "例: 「排水構造物工」→ 基礎砕石・管渠工・集水桝工 等")
            _render_alias_section(
                raw=raw_mr,
                json_path=mr._JSON_PATH,
                reload_fn=mr.reload,
                sections=[
                    ("implicit_dekigata", "出来形",
                     "数量総括表のキーワード → 自動追加する出来形管理工種",
                     "キーワード", "追加するDB工種名"),
                ],
                table_id="mr_id",
            )
            st.divider()
            st.markdown("##### 品質管理の自動追加")
            st.caption("数量総括表に特定のキーワードが含まれる場合、"
                       "関連する品質管理工種を自動的に追加します。"
                       "例: 「カルバート工」→ セメント・コンクリート + プレキャストコンクリート製品")
            _render_alias_section(
                raw=raw_mr,
                json_path=mr._JSON_PATH,
                reload_fn=mr.reload,
                sections=[
                    ("implicit_hinshitsu", "品管",
                     "数量総括表のキーワード → 自動追加する品質管理工種",
                     "キーワード", "追加するDB工種名"),
                ],
                table_id="mr_ih",
            )
        ...
        with sub_tabs[4]:
            st.markdown("##### 品質管理 工種の表示名変換")
            st.caption("Excel出力時にDB上の工種名を別の表示名に変換します。"
                       "例: 「固結工」→「中層混合処理」")
            _render_alias_section(
                raw=raw_mr,
                json_path=mr._JSON_PATH,
    ...
    with main_tabs[1]:
        ...
        with sub_tabs_p[2]:
            st.markdown("##### 品質管理写真の自動追加")
            st.caption("数量総括表にキーワードが含まれる場合、"
                       "品質管理写真の工種を撮影箇所一覧に自動追加します。"
                       "例: 「河川土工」→ セメント・コンクリート + 下層路盤 + アスファルト舗装")
            _render_alias_section(
                raw=raw_pa,
                json_path=pa._JSON_PATH,
                reload_fn=pa.reload,
                sections=[
                    ("implicit_photo", "自動追加", "キーワード → 自動追加する品質管理写真の工種",
                     "キーワード", "追加工種名"),
                ],
                table_id="pa_ip",
            )
